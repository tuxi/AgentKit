...
> 状态：Active — Runtime M0–M4、AgentKit W2 与 W3 客户端验收已完成；归档契约和服务端原子删除保护仍是生产阻断项
>
> 适用端：macOS、iOS
>
...
AgentKit 的真实 Runtime 黑盒矩阵通过临时 Git 仓库、独立 SQLite 和真实模型 turn 验证以上路径。当前 Runtime 的 `turn_queued` 已能证明 shared workspace 串行，但 wire 事件尚未填充 `reason`；在 Runtime 补齐 `workspace_lease/global_capacity` 前，客户端只能展示通用排队文案。
...
当前 Runtime 尚无 conversation archive/list-archived/restore 契约，AgentKit 不在本地伪造服务端归档事实。归档入口待 Runtime 提供持久化 archive endpoint 后复用同一 Worktree disposition 流程。
