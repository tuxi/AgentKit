...
> 状态：Active — Runtime M0–M4、AgentKit W2/W3、Conversation Archive/Restore v1 与 ConversationLocalStateStore v1 已完成
>
> 适用端：macOS、iOS
>
...
- Runtime `3ff19eb` 通过 `conversation_archive_v1` 开放持久化 archive、restore 和归档列表。AgentKit 仅在能力为 true 时显示入口，不在本地伪造归档状态。
- 归档 conversation 默认保留 worktree、消息、事件、terminal sequence 与未读游标；归档详情以只读方式打开，恢复后才重新建立控制连接。
- archive、restore、delete 与 turn acceptance 共用服务端 session operation gate；客户端快照检查只负责即时 UX，`409 conversation_in_use` 仍是最终权威结果。
...
## 8. ConversationLocalStateStore（v1 已完成）
...
v1 已引入统一持久化边界。Composer 的 `@State` 只负责当前帧 UI，SQLite 才是跨 View/App 生命周期的设备本地真相：
...
public protocol ConversationLocalStateStore: Sendable {
    ...
    func updateState(for key: ConversationLocalStateKey, _ update: @Sendable (inout ConversationLocalState) -> Void) throws
...
- 默认数据库位于 Application Support 的 `CodeAgent/conversation-local-state-v1.sqlite`，启用 WAL、`synchronous=NORMAL` 与 busy timeout。
- `SessionDraft.id` 是稳定本地主键；workspace、幂等 `clientRequestID`、Worktree 选择/base ref/suggested name 与 Composer 一起恢复。
- 文本变化 400ms debounce，scene inactive/background 和 Workspace 消失时立即保存/flush。
- draft→session 在 SQLite 事务中迁移，并留下 migrated tombstone；旧 SwiftUI View 的迟到 `onDisappear` 写入不能复活草稿。
- 首条消息被本地 session channel 接收后才迁移并清空文本；创建或发送失败继续保留草稿。
- `ModelSettingsStore` 将旧 per-conversation UserDefaults 字典迁入本 Store；App 级 `lastSelectedModel` 仍保留为新会话默认值。
- `ConversationLocalStateAttentionReadStore` 接管 terminal/approval/read 游标，并从旧 UserDefaults 建立兼容基线。
- 归档保留本地状态；Runtime 确认永久删除后才删除 `session:<id>`。
- iOS 数据库文件使用 `completeUntilFirstUserAuthentication` 文件保护。
...
Runtime `6e4875a` 已为永久删除增加原子 `conversation_in_use` 保护；`3ff19eb` 进一步开放持久化归档分区。AgentKit 的默认列表、归档列表、详情只读模式、恢复入口和永久删除均消费 Runtime 事实；归档不会触发 Worktree remove，永久删除仍复用显式 Worktree disposition 流程。
