...
>
> Managed Worktree 配套契约：[Code-Agent Managed Worktree v1 要求](code-agent-managed-worktree-v1.md)
...
### 6.4 已供给 Worktree 与 Managed Worktree

`execution_policy=isolated_worktree` 本身只表示 `workspace_path` 已经指向独立 checkout，
不授权 Runtime 自动执行 `git worktree add`。Runtime 托管创建必须同时满足：
...
未显式请求时，创建 shared workspace session 对 Git 必须零副作用。Managed worktree 的
默认项目内路径、创建幂等、真实路径隔离、崩溃恢复和 dirty-safe 清理遵循独立 v1 契约；
仅实现 Git 命令调用不得开放 capability。
...
`GET /v1/activity` 已合并持久化 lifecycle、scheduler、broker 和 terminal attention。声明
`session_attention_delta_v1=true` 后，客户端首次请求完整基线，随后使用
`?since_sequence=<cursor>` 获取有界增量并按 session 合并；旧 Runtime 仍使用全量快照。
...
### R6：显式 Managed Worktree

- 保持 session 创建默认 `shared_workspace`，只处理显式 opt-in；
- 持久化 worktree reservation、checkout identity、branch、base ref 和 lifecycle；
- 排除项目内 `.codeagent/worktrees/`，防止 base workspace 工具读取其他会话 checkout；
- 实现 restart reconciliation、missing/orphan 诊断和 dirty-safe remove；
- 真实双 worktree、重启、幂等和 race 验收通过后开启 `managed_worktree_v1`。
